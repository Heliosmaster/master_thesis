...
distinct = unique(nz_sorted);
n_distinct = length(distinct);

index = 1; %index on the nz_sorted array
index2 = 1; %index on the n_distinct

% indices for assigning to Ar or Ac
index_r = 1;
index_c = 1;

while(index2 <= n_distinct && index <= m+n)
    id = sort_index(index);
    if
end
