...
int AddDummiesToSparseMatrix(struct sparsematrix *pM, int isFor2D)  {
    ...
    if(isFor2D){
		...
		for(t=0;t<n;t++){
			if(isRowEmpty[t]==TRUE || isColEmpty[t]==TRUE){
				pM->dummy[t] = FALSE;
                pM->NrDummies--; /* no dummy needed */
			}
		}
		free(isRowEmpty);
		free(isColEmpty);
...
int CreateInitialTwoDimDistribution2(struct sparsematrix *pM, long *mid){
    ...
    */
    ...
    long *rowCnt, *colCnt, *Cnt, *J, *Index, *rowStart, *colStart,
         *rowComplete, *colComplete, maxCnt;
    long m, n, nz, t, i, j, k, NrNeRows, NrNeCols, start, dir, left, right;
    double tmpRe = 0.0, tmpIm = 0.0 ;
...
int ConvertSparseMatrixToTwoDimMatrix(struct sparsematrix *pM,struct sparsematrix *pM2D,long mid){
    ...
    if( !MMSparseMatrixAllocateMemory(pM2D) ) {
        fprintf( stderr, "Error: could not allocate sparse matrix (SparseMatrix.c::ConvertSparseMatrixToTwoDimMatrix)!\n" );
        return FALSE;
    }

    if( !MMWeightsInit( pM2D ) ) {
        fprintf(stderr, "ConvertSparseMatrixToTwoDimMatrix(): error during row/column weight initialisation!\n" );
        return FALSE;
    }
    
    long i;
    for(i=0;i<pM2D->m;i++) pM2D->RowMark[i]=FALSE;
    for(i=0;i<pM2D->n;i++) pM2D->ColMark[i]=FALSE;
    for(i=0;i<pM->NrNzElts;i++){
		if(i<mid){
			pM2D->i[i]=pM->i[i]+pM->n;
			pM2D->j[i]=pM->j[i];
		}else{
			pM2D->i[i]=pM->j[i];
			pM2D->j[i]=pM->i[i]+pM->n;
		}
		if (pM->MMTypeCode[2] != 'P')
			pM2D->ReValue[i] = pM->ReValue[i];
		if (pM->MMTypeCode[2] == 'C')
			pM2D->ImValue[i] = pM->ImValue[i];
	}
    AddDummiesToSparseMatrix(pM2D,TRUE);
    return TRUE;
}

int ConvertTwoDimMatrixDistributionForNextRun(struct sparsematrix *pM2D,long mid2D,long n){
	long *rowP0 = (long *)malloc(pM2D->m*sizeof(long));
	long *rowP1 = (long *)malloc(pM2D->m*sizeof(long));
	long i;
	for(i=0;i<pM2D->m;i++){
		rowP0[i]=0;
		rowP1[i]=0;
	...
	for(i=0;i<pM2D->NrNzElts;i++){
		rowI = pM2D->i[i];
		colI = pM2D->j[i];
		if(rowI==colI) continue;
		//if(rowP0[rowI]==0 || rowP1[rowI]==0){
			if(rowI>=n && i<mid2D){
				pM2D->i[i] = pM2D->i[i]-n;
				pM2D->j[i] = pM2D->j[i];
				long tmp = pM2D->i[i];
...
int ConvertTwoDimMatrixDistributionToSparseMatrix(struct sparsematrix *pM,long lo,long hi,struct sparsematrix *pM2D,long *mid,long mid2D){
	long new,
	...
	for(i=0;i<pM2D->NrNzElts;i++){
		...
		if (P == 0) 
			...
			new = right;
		if (new < lo || new > hi) {
			fprintf(stderr, "ConvertTwoDimMatrixDistributionToSparseMatrix(): nonzero outside range!\n");
			return FALSE;
		}
		if(pM2D->i[i]>=pM->n){
			pM->i[new] = pM2D->i[i]-pM->n;
			pM->j[new] = pM2D->j[i];
		}else{
			pM->i[new] = pM2D->j[i]-pM->n;
			pM->j[new] = pM2D->i[i];
		}
		if (pM->MMTypeCode[2] != 'P')
			pM->ReValue[new] = pM2D->ReValue[i];
		if (pM->MMTypeCode[2] == 'C')
			pM->ImValue[new] = pM2D->ImValue[i];
