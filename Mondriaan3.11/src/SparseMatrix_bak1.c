...
// Versie die dynamisch verdeeld
// (op basis van zo min mogelijk rows/cols uitschakelen)
int CreateInitialTwoDimDistribution2(struct sparsematrix *pM,long *mid){
	long nnz = pM->NrNzElts;
	if(nnz==0){
		*mid=0;
		return TRUE;
	}
	long *rowCnt = (long *)malloc(pM->m*sizeof(long));
	long *colCnt = (long *)malloc(pM->n*sizeof(long));
	long *cnt = (long *)malloc((pM->m+pM->n)*sizeof(long));
	long *ids = (long *)malloc((pM->m+pM->n)*sizeof(long));
	long *nextids = (long *)malloc((pM->m+pM->n)*sizeof(long));
	long nextCnt;
	long nNext;
	long *frees = (long *)malloc((pM->m+pM->n)*sizeof(long));
	long *iBackup = (long *)malloc(nnz*sizeof(long));
	long *jBackup = (long *)malloc(nnz*sizeof(long));
	long *reBackup;
	long *imBackup;
	if (pM->MMTypeCode[2] != 'P')
		reBackup = (long *)malloc(nnz*sizeof(long));
	if (pM->MMTypeCode[2] == 'C')
		imBackup = (long *)malloc(nnz*sizeof(long));
	
	long t;
	long new,
         left = 0,   /* lowest index of nonzero in range */ 
         right = nnz-1; /* highest index */
	long i,j;
	for(t=0;t<pM->m;t++){
		rowCnt[t]=0;
		cnt[t]=0;
	...
	for(t=0;t<nnz;t++){
		...
		jBackup[t]=pM->j[t];
		if (pM->MMTypeCode[2] != 'P')
			reBackup[t] = pM->ReValue[t];
		if (pM->MMTypeCode[2] == 'C')
			imBackup[t] = pM->ImValue[t];
	}
	for(t=0;t<pM->m+pM->n;t++){
		if(cnt[t]<=1) cnt[t]=-1;
	}
	
	//RandomPermute(cnt,ids,NULL,NULL,0,pM->m+pM->n-1);
	//quicksort(cnt,ids,0,pM->m+pM->n-1);
	long u,v,nFixed;
	while(TRUE){
	//for(t=pM->m+pM->n-1;t>=0;t--){
	//for(t=0;t<pM->m+pM->n;t++){
		nextCnt = LONG_MAX;
		nNext=0;
		nFixed=0;
		for(t=0;t<pM->m+pM->n;t++){
			if(frees[t]==FALSE) nFixed++;
			if(cnt[t]<0) continue;
			if(frees[t]==TRUE){
				if(cnt[t]<nextCnt){
					nNext=1;
		...
		if(nNext==0) break;
		long tN = Random1(0,nNext-1);
		long nextID = nextids[tN];
		frees[nextID]=FALSE;
		if(nextID<pM->m){
			new = right;
			for(u=0;u<nnz;u++){
				...
				if(i==nextID){
					...
					pM->j[new]=j;
					if (pM->MMTypeCode[2] != 'P')
						pM->ReValue[new] = reBackup[u];
					if (pM->MMTypeCode[2] == 'C')
						pM->ImValue[new] = imBackup[u];
					right--;
					new=right;
					iBackup[u]=-1;
					jBackup[u]=-1;
	...
	for(t=0;t<nnz;t++){
		...
		j = jBackup[t];
		if(i==-1||j==-1) continue;
		if((rowCnt[i]>=colCnt[j] && colCnt[j]>1)){
			new = left;
		}else{
			new = right;
		}
		...
		pM->j[new]=j;
		if (pM->MMTypeCode[2] != 'P')
			pM->ReValue[new] = reBackup[t];
		if (pM->MMTypeCode[2] == 'C')
			pM->ImValue[new] = imBackup[t];
