...
int AddDummiesToSparseMatrix(struct sparsematrix *pM, int nonempty)  {
    ...
    /* This function adds dummies to a square matrix to make its diagonal 
       ...
       If nonempty is true, a dummy a(i,i) is only created if
       both matrix row i and column i are nonempty.
  
    */

    long t, tt;
    long i, j, n, NrNzEltsNew;
    int *isRowEmpty, *isColEmpty;
    ...
    if(nonempty){
        ...
        isRowEmpty = (int *) malloc(n * sizeof(int));
        isColEmpty = (int *) malloc(n * sizeof(int));
        if (isRowEmpty == NULL || isColEmpty == NULL) {
            fprintf(stderr, "AddDummiesToSparseMatrix(): Not enough memory!\n");
            return FALSE;
        }
        ...
        for(i=0; i<n; i++){
            if(isRowEmpty[i] || isColEmpty[i]){
                /* no dummy needed because row or column is empty */
                pM->dummy[i] = FALSE;
                pM->NrDummies--;
            }
        }
    ...
    for (t = 0; t < pM->NrNzElts; t++) {
        ...
        if (i == j)
            if (pM->dummy[i]) {
                /* no dummy needed because of actual diagonal nonzero */
                pM->dummy[i] = FALSE;
                pM->NrDummies--; 
            }
    }
    ...
    if (tt != NrNzEltsNew){
        fprintf(stderr, "AddDummiesToSparseMatrix(): internal error!\n");
        return FALSE;
    }
...
int CreateInitialTwoDimDistribution(struct sparsematrix *pM, long *mid){
    ...
    */
    ...
    if (rowCnt == NULL || colCnt == NULL){
        fprintf(stderr, "CreateInitialTwoDimDistribution(): Not enough memory!\n");
        return FALSE;
    }
    ...
    while(left <= right){ /* nonzeros in range left..right still to be split */
        ...
        if(rowCnt[i] > colCnt[j] || (rowCnt[i] == colCnt[j] && dir == COL)){
...
} /* end CreateInitialTwoDimDistribution */


int CreateInitialTwoDimDistribution2(struct sparsematrix *pM, long *mid){

    /* This function creates a twodimensional distribution of the nonzeros
       of the sparse matrix M by splitting them into two sets, with indices
       in the ranges 0..mid-1 and mid..nz(M)-1, respectively.
 
       The first group contains the nonzeros a(i,j) for which c(j) < r(i),
       i.e. the number of nonzeros in column j is less than the number in row i.
       The second group contains the rest. Ties are broken based on
       the dimensions of the matrix.
    */

    long *rowCnt, *colCnt, *J, *Cnt, *Index, *rowStart, *colStart,
         *rowComplete, *colComplete;
    long m, n, nz, t, i, j, k, NrNeRows, NrNeCols, maxCnt, start, dir, left, right;
    long nrow_complete, ncol_complete ;
    double tmpRe = 0.0, tmpIm = 0.0 ;
    ...
    rowCnt = (long *)malloc(m*sizeof(long));
    colCnt = (long *)malloc(n*sizeof(long));
    J = (long *)malloc((m+n)*sizeof(long));
    Cnt = (long *)malloc((m+n)*sizeof(long));
    Index = (long *)malloc(2*nz*sizeof(long));
    rowStart = (long *)malloc(m*sizeof(long));
    colStart = (long *)malloc(n*sizeof(long));
    rowComplete= (long *)malloc(m*sizeof(long));
    colComplete= (long *)malloc(n*sizeof(long));
    if (rowCnt == NULL || colCnt == NULL || J == NULL ||
        Cnt == NULL || Index == NULL || rowStart == NULL || colStart == NULL ||
        rowComplete == NULL || colComplete == NULL ){
        fprintf(stderr, "CreateInitialTwoDimDistribution(): Not enough memory!\n");
        return FALSE;
    }

    /* Count the nonzeros in the rows and columns
       and initialise completeness to TRUE */
    maxCnt = 0 ; /* maximum number of nonzeros in a row or column */
    for(i=0; i<m; i++){
        rowCnt[i] = 0;
        rowComplete[i] = TRUE ;
    }
    ...
    /* Sort the rows and columns by increasing number of nonzeros */

    /* Insert nonempty rows into index array J */
    t = 0 ;
    for(i=0; i<m; i++){
        if (rowCnt[i] > maxCnt)
            maxCnt = rowCnt[i] ;
    ...
    /* Insert nonempty columns into index array J */
    for(j=0; j<n; j++){
        if (colCnt[j] > maxCnt)
            maxCnt = colCnt[j] ;
        Cnt[m+j] = colCnt[j] ;
    ...
    CSort(J, Cnt, maxCnt, 0, NrNeRows + NrNeCols-1) ;

    /* Initialise rowStart and colStart */
    start = 0 ;
    for(t=0; t<NrNeRows + NrNeCols; t++){
        if (J[t] < m) /* J[t] is a row index */{
            i = J[t] ;
            rowStart[i] = start ;
            start += rowCnt[i] ; 
        } else {
    ...
    /* For each row in sorted order, insert the column indices into array Index, 
      and for each column the row indices */
    ...
    /* Reinitialise rowStart and colStart */
    start = 0 ;
    for(t=0; t<NrNeRows + NrNeCols; t++){
        if (J[t] < m) /* J[t] is a row index */{
            i = J[t] ;
    ...
    /* Assign complete rows and columns as long as possible */
    nrow_complete = 0;
    ncol_complete = 0;
    for(t=0; t<NrNeRows + NrNeCols; t++){
        if (J[t] < m && rowComplete[J[t]] ){
            i = J[t] ;
            start = rowStart[i] ;
            for(k=start; k<start+rowCnt[i]; k++)
                colComplete[Index[k]] = FALSE ;
            nrow_complete++ ;
        } else if (J[t] >= m && colComplete[J[t]-m] ){ 
            j = J[t]-m ;
            start = colStart[j] ;
            for(k=start; k<start+colCnt[j]; k++)
                rowComplete[Index[k]] = FALSE ;
            ncol_complete++ ;
        }
    }
    //printf("nonempty rows = %ld complete rows = %ld \n", NrNeRows, nrow_complete) ;
    //printf("nonempty cols = %ld complete cols = %ld \n", NrNeCols, ncol_complete) ;

    /* Choose uniform direction for ties between row and column count */
    if (NrNeCols > NrNeRows)
        dir = COL; /* nonempty columns are shorter on average */
    ...
    while(left <= right){ /* nonzeros in range left..right still to be split */
        ...
        if(colComplete[j] || (!rowComplete[i] && 
              ( rowCnt[i] > colCnt[j] || (rowCnt[i] == colCnt[j] && dir == COL)))){
            left++; /* column wins */
        } else {    /* row wins */
            /* Copy right values into left */
            pM->i[left] =   pM->i[right];
...
int ConvertSparseMatrixToTwoDimMatrix(struct sparsematrix *pM,
                                      struct sparsematrix *pM2D, long mid){

    /* This function converts an m by n sparse matrix A = M into 
       an (m+n) by (m+n) matrix B = M2D defined by

                 [ I_n   (A_1)^T ]
             B = [               ]
                 [ A_0      I_m  ]

       where A = A_0 + A_1, and the nonzeros of A_0 are given
       by the nonzero range 0..mid-1 of A, and those of A_1 by the range
       mid..nz-1 of A.

       If A is a distributed matrix (MMTypeCode[0]= 'D') or weighted matrix
       (MMTypeCode[0]= 'W'), this information is not transferred to B.
       The same holds for any symmetry (MMTypeCode[3] = 'S', or so on).
    */

    long i, t ;
    
    if( !MMSparseMatrixInit(pM2D) ) {
        fprintf(stderr, "Error: could not initialise sparse matrix for Mondriaan!\n" );
        return FALSE;
    }
    ...
    pM2D->MMTypeCode[ 0 ] = 'M';
    pM2D->MMTypeCode[ 1 ] = 'C';
    pM2D->MMTypeCode[ 2 ] = pM->MMTypeCode[2];
    pM2D->MMTypeCode[ 3 ] = 'G';
    pM2D->m = pM->m+pM->n;
    pM2D->n = pM->m+pM->n;
    pM2D->NrNzElts = pM->NrNzElts;
    
    if( !MMSparseMatrixAllocateMemory(pM2D) ) {
        fprintf(stderr, "ConvertSparseMatrixToTwoDimMatrix(): Error, ");
        fprintf(stderr, "could not allocate sparse matrix!\n");
        return FALSE;
    }

    if( !MMWeightsInit( pM2D ) ) {
        fprintf(stderr, "ConvertSparseMatrixToTwoDimMatrix(): ");
        fprintf(stderr, "Error during row/column weight initialisation!\n" );
        return FALSE;
    }
    
    /* Initialise the marker arrays to false */
    for(i=0; i<pM2D->n; i++){ /* M2D is square */
        pM2D->RowMark[i]=FALSE;
        pM2D->ColMark[i]=FALSE;
    }

    /* Enter the nonzeros into A_0 or (A_1)^T */
    for(t=0; t<pM->NrNzElts; t++){
        if(t<mid){
            /* Insert a(i,j) as b(i+n,j) into A_0 */ 
            pM2D->i[t] = pM->i[t] + pM->n;
            pM2D->j[t] = pM->j[t];
        } else {
            /* Insert a(i,j) as b(j,i+n) into (A_1)^T */ 
            pM2D->i[t] = pM->j[t];
            pM2D->j[t] = pM->i[t] + pM->n;
        }
        if (pM->MMTypeCode[2] != 'P')
            pM2D->ReValue[t] = pM->ReValue[t];
        if (pM->MMTypeCode[2] == 'C')
            pM2D->ImValue[t] = pM->ImValue[t];
    }
    
    /* Add diagonal matrices I_n and I_m */
    AddDummiesToSparseMatrix(pM2D, TRUE);

    return TRUE;

} /* end ConvertSparseMatrixToTwoDimMatrix */


int ConvertTwoDimMatrixDistributionForNextRun(struct sparsematrix *pM2D,
                                              long mid2D, long n){

    /* This function converts an (m+n) by (m+n) matrix B = M2D
       which was split into two parts, and is in the form

                 [ I_n   (A_1)^T ]
             B = [               ]
                 [ A_0      I_m  ]

       into a new matrix B' of the same form. The matrices A_0 and A_1
       are of size m by n. The nonzeros of A_0 assigned to part 0
       will stay in A_0'. The nonzeros b(i,j) of (A_1)^T assigned to part 0
       are moved into A_0' as b(j,i). Thus all nonzeros assigned to part 0
       are collected in A_0'.

       Similarly, the nonzeros assigned to part 1 are collected in (A_1')^T.
        
       For the matrix B, the nonzeros of the range are assumed
       to have been split into two parts:
           0..mid2D-1 belonging to part 0 (representing one processor),
           mid..nz(B)-1 belonging to part 1 (representing another processor).

       Since the input and output matrix have the same form, 
       this function can be used in repeated runs of a partitioner.

    */

    long i, j, t, tmp, *rowP0, *rowP1;

    rowP0 = (long *)malloc(pM2D->m*sizeof(long));
    rowP1 = (long *)malloc(pM2D->m*sizeof(long));
    if (rowP0 == NULL || rowP1 == NULL){
        fprintf(stderr, "ConvertTwoDimMatrixDistributionForNextRun(): \n");
    ...
    /* Count the number of nonzeros in each row assigned to parts 0 and 1 */
    for(t=0; t<mid2D ; t++)
        rowP0[pM2D->i[t]]++;
    for(t=mid2D; t<pM2D->NrNzElts; t++)
        rowP1[pM2D->i[t]]++;

    for(t=0; t<pM2D->NrNzElts; t++){
        i = pM2D->i[t];
        j = pM2D->j[t];
        if(i==j) /* dummy nonzero */
            continue;
        //if(rowP0[i]==0 || rowP1[i]==0){
            if( (i>=n && t<mid2D) || (i<n && t>=mid2D) ){
    ...
    RemoveDummiesFromSparseMatrix(pM2D);
    AddDummiesToSparseMatrix(pM2D,TRUE);
...
} /* end ConvertTwoDimMatrixDistributionForNextRun */


int ConvertTwoDimMatrixDistributionToSparseMatrix(struct sparsematrix *pM,
    long lo, long hi, struct sparsematrix *pM2D, long *mid, long mid2D){
    ...
    /* This function converts an (m+n) by (m+n) matrix B = M2D
       ...
       into an m by n sparse matrix A = A_0 + A_1 = M, also split into two parts.
       ...
       For the matrix A, the nonzeros will also be split into two parts:
           lo..mid-1 belonging to part 0,
           mid..hi belonging to part 1.
    ...
    */
    ...
    new = lo; /* index of new nonzero in A */ 
    nz0 = 0;  /* number of nonzeros of part 0 */ 
    n = pM->n ;

    for(t=0; t<pM2D->NrNzElts; t++){

        /* Discard diagonal nonzero */
        if(pM2D->i[t] == pM2D->j[t])
            continue;
        ...
        if (new > hi) {
            fprintf(stderr, "ConvertTwoDimMatrixDistributionToSparseMatrix(): ");
            fprintf(stderr, "nonzero outside range!\n");
            return FALSE;
        }
        ...
        if (pM->MMTypeCode[2] != 'P')
            pM->ReValue[new] = pM2D->ReValue[t];
        if (pM->MMTypeCode[2] == 'C')
            pM->ImValue[new] = pM2D->ImValue[t];
...
} /* ConvertTwoDimMatrixDistributionToSparseMatrix */
