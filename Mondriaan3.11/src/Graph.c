...
int SparseMatrix2BiPartHyperGraph(const struct sparsematrix *pM, 
    ...
    */
    ...
    long *mgNeR,*mgNeC,nDummies,nVertices,nNets;
    int *Diag;
    ...
    if (dir == MEDIUMGRAIN) {
		...
		for(t=pM->mgMid;t<pM->NrNzElts;t++){ /* in A^r */
			i = pM->i[t];
			j = pM->j[t];
    ...
    if (pHG->SplitDir == ROW) {
        ...
        for (i = 0; i < pM->n; i++) {
            if (mgNeR[i] > 0) {
                ...
                pHG->N[t].iStartP0 = pHG->N[t].iStartP1 = pHG->N[t].iEnd = pin_index;
                pHG->N[t].dir = ROW;
                pHG->N[t].Free = (pM->ColMark[i] == 1 ? TRUE : FALSE);
                if (mgNeC[i]!=0){
        ...
        for (i = pM->n; i < pM->m+pM->n; i++) {
            if (mgNeR[i] > 0) {
                ...
                pHG->N[t].iStartP0 = pHG->N[t].iStartP1 = pHG->N[t].iEnd = pin_index;
                pHG->N[t].dir = ROW;
                pHG->N[t].Free = (pM->RowMark[i-pM->n] == 1 ? TRUE : FALSE);
                if (mgNeC[i]!=0){
					pin_index += mgNeR[i]+1; /* Added dummy */
				}else{
					pin_index += mgNeR[i];
