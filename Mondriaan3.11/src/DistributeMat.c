...
int SplitMatrixKLFM(struct sparsematrix *pT, int k, int i, int dir,
    ...
    */
    ...
    long lo, hi, mid, mid2D, nz, weight, nbytes, nbytesd,
         bestNNZ, bestcomm, bestmid2D ;
    long *besti=NULL, *bestj=NULL ;
    double *bestRe=NULL, *bestIm=NULL ;
    int j, bestdir=ROW, iter, niters=1 ;
    ...
    */
    ...
    if (pOptions->SplitStrategy == LocalBest ||
    ...
    } else if(pOptions->SplitStrategy == TwoDim){

#ifdef INFO2
        printf("**** Split extended twodim matrix in column direction \n");
#endif

        A.mgMid = hi+1;
        if(!CreateInitialTwoDimDistribution(&A, &(A.mgMid))){
            fprintf(stderr, "SplitMatrixKLFM(): Unable to perform initial twodim split!\n");
            return FALSE;
        }
		...
		if (!SparseMatrix2BiPartHyperGraph(&A, MEDIUMGRAIN, pOptions, &HG)) {
			fprintf(stderr, "SplitMatrixKLFM(): Unable to convert twodim matrix to hypergraph!\n");
			return FALSE;
		}
